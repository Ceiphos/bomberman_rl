...
def setup(self):
    ...
    if self.train or not os.path.isfile(MODEL_NAME):
        ...
        self.eps = epsilonPolicy([0, 1000], [0.95, 0.4], [1 / 100, 1 / 200], [0.05, 0.1])
    else:
...
def state_to_features(game_state: dict, logger) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    bomb_times = [t for _, t in bombs]
    bomb_spots = [pos for pos, _ in bombs]
    bomb_field = np.ones_like(field) * -1  # Initialize the bomb field with a large value, to distiguish from a bomb
    for (x, y), t in bombs:
        ...
        for tile in single_bomb_field:
            if (bomb_field[tile] == -1):
                bomb_field[tile] = t
            else:
                bomb_field[tile] = min(t, bomb_field[tile])

    if bomb_field[position[0], position[1]] > -1:  # if in bomb field, 1, else 0
        features.append(1)
    else:
        features.append(0)
    if bomb_possible:
    ...
    for tile in tiles_to_check:
        if field[tile] == 1:
            ...
            threatened_enemy += 1
    features.append(destroyable_crates / 12)  # We can destroy a maximum of 12 crates with one bomb (but only with cheating)
    features.append(threatened_enemy / 3)  # Maximum of 3 other players

    danger_score_position = bomb_field[position]
    features.append(danger_score_position / 3)  # Maximum value of 3
    ...
    if nearest_coin is not None:
        path = findPath(walk_field, position, nearest_coin)
        if path is not None:
            features.append((len(findPath(walk_field, position, nearest_coin)) - 1))  # Normalize by the diagonal of the field size
        else:
