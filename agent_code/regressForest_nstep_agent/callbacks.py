...
MODEL_NAME = "regressForest_nstep_model.pt"
FEATURE_SIZE = 36
escape_path = []
...
def setup(self):
    ...
    if self.train or not os.path.isfile(MODEL_NAME):
        ...
        self.model = Model()
        self.eps = epsilonPolicy([0, 200, 1000, 2000], [1, 1, 0.7, 0.3], [0, 1 / 300, 1 / 300, 1 / 200], [0.05] * 4)
    else:
...
def act(self, game_state: dict) -> str:
    ...
    if self.train and random.random() < self.eps.epsilon(round):
        ...
        return np.random.choice(ACTIONS, p=([.2] * 4 + [.1] * 2))
    else:
